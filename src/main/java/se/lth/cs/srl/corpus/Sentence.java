...
public class Sentence extends ArrayList<Word> {
	...
	public void buildDependencyTree(){
		for(int i=1;i<size();++i){
			Word curWord = get(i);
		//	System.out.println(curWord.getHeadId());
		//	System.out.println(curWord.toString());
			if (curWord.getHeadId() <0)
				curWord.setHead(get(0));
			else
	...
	public static Sentence newSentence(String[] lines, boolean useGold){
		Sentence ret=new Sentence();
		Word nextWord;
		int ix=1;
		for(String line:lines){
			...
			String[] cols=WHITESPACE_PATTERN.split(line);
			if(cols[12].charAt(0) == 'Y'){
				Predicate pred=new Predicate(cols,ret,ix++, useGold);
				if (cols[12].endsWith("I")){
					pred.isIgnore = true;
				}
				ret.addPredicate(pred);
				...
				nextWord=new Word(cols,ret,ix++, useGold);
			}
			ret.add(nextWord);
		}
		ret.buildDependencyTree();
	...
	public static Sentence newSRLOnlySentence(String[] lines, boolean useGold){
		Sentence ret=new Sentence();
		Word nextWord;
		int ix=1;
		for(String line:lines){
	...
	public static Sentence newSentence(Sentence s){
		Sentence ret=new Sentence();
		for (Word w : s){
			if (w.isBOS) continue;
			Word wprime = new Word(w);
				ret.add(wprime);
		}

		for (int i=0;i<s.getPredicates().size();i++){
			Predicate p = s.getPredicates().get(i);
			ret.makePredicate(s.indexOf(p));
			((Predicate)ret.get(s.indexOf(p))).setSense(p.getSense());
			for (Word w : p.getArgMap().keySet()){
				((Predicate)ret.get(s.indexOf(p))).addArgMap(ret.get(s.indexOf(w)), p.getArgumentTag(w));
			}
