...
//	}
	...
	public void addInstance(int label, Collection<Integer> indices,
		...
		out.print(' ');
		problemWriter.writeIndices(indices);
		problemWriter.writeNumericIndices(we, offset);
		out.println();
	...
	private static abstract class ProblemWriter {
		...
		/***
		 * we add new method here
		 * @param numeric_indices
		 * @param offset
		 */
		abstract void writeNumericIndices(float[] numeric_indices, int offset);
		//////////////////
	}
	private static class HistogramProblemWriter extends ProblemWriter{
		...
		void writeNumericIndices(float[] numeric_indices, int offset)
		{
			int i =1;
			for (Float d : numeric_indices)
			{
