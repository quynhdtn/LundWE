...
public abstract class WeightVector implements Serializable {
	...
	public abstract double[] computeAllProbs(Collection<Integer> ints, float[] we, int offset);

	public abstract short computeBestClass(Collection<Integer> ints);

	public abstract  short computeBestClass(Collection<Integer> indices, float[] we, int offset) ;
	...
	public abstract static class BinaryVector extends WeightVector {
		...
		/***
		 * we add new method here
		 * @param ints
		 * @param we
		 * @param offset
		 * @return
		 */
		protected abstract double computeScore(Collection<Integer> ints, float[] we, int offset);
		...
		public double[] computeAllProbs(Collection<Integer> ints, float[] we, int offset){
			double[] ret=new double[2];
			double prob=(1.0/(1.0+Math.exp(-computeScore(ints, we, offset))));
			ret[0]=prob;
			ret[1]=1.0-prob;
			return ret;
		}
		...
		public short computeBestClass(Collection<Integer> ints, float[] we, int offset) {
			if(computeScore(ints, we, offset)>0)
				return 0;
			else
				return 1;
		}
	...
	public static class BinaryLibLinearVector extends BinaryVector {
		 ...
		 */
		@Override
		protected double computeScore(Collection<Integer> ints, float[] we, int offset) {
			double sum=(bias>0.0?bias*weights[features]:0.0);
			for(Integer i:ints){
				if((i-1)<features)
					sum+=weights[i-1];
	...
	public static class BinarySparseVector extends BinaryVector {
		...
		protected double computeScore(Collection<Integer> ints, float[] we, int offset) {
			...
			for (int i =0;i<we.length;i++){
				//if (weightMap.containsKey(i+offset-1))
				sum+=weightMap.get(i+offset)*we[i];
			}
	...
	public abstract static class MultipleVector extends WeightVector {
		...
		public double[] computeAllProbs(Collection<Integer> ints, float[] we, int offset) {
			double[] ret=new double[classes];
			double[] scores=computeScores(ints, we, offset);
			double sum=0.0;
			for(short i=0;i<classes;++i){
				ret[i]=(1.0/(1.0+Math.exp(-scores[i])));
				sum+=ret[i];
		...
		protected abstract double[] computeScores(Collection<Integer> ints, float[] we, int offset) ;
	...
	public static class MultipleLibLinearVector extends MultipleVector {
		...
		@Override
		protected double[] computeScores(Collection<Integer> ints, float[] we, int offset) {
			double[] ret=new double[classes];
			for(int i=0;i<classes;++i){
				double curvalue=(bias>0?bias*weights[i][features]:0);
				for(Integer in:ints){
				...
				for (int j =0;j<we.length;j++){
					//if (weightMap.containsKey(i+offset-1))
					curvalue+=weights[i][j+offset]*we[j];
				}
	...
	public static class MultipleSparseVector extends MultipleVector {
		...
		protected double[] computeScores(Collection<Integer> ints, float[] we, int offset) {
			...
			for(int i=0;i<classes;++i){
				...
				for (int j =0;j<we.length;j++){
					//if (weightMap.containsKey(i+offset-1))
					curvalue+=weightMap.get(j+offset).weights[i]*we[j];
				}
