...
import liir.nlp.wesrl.we.WEFeatureSimple;
...
//		extractLabels(sentences, featureSets.get(Step.pd).POSPrefixes); //TODO, sort of nasty.
	...
	private void setup(Map<Step,FeatureSet> featureSets,PredicateReference predicateReference,List<String> argLabels){
		...
		HashMap<String, WEFeature> piwes=new HashMap<>();
		HashMap<String, WEFeature> pdwes=new HashMap<>();

		HashMap<String, WEFeature> aiwes=new HashMap<>();

		HashMap<String, WEFeature> acwes=new HashMap<>();

		WEFeature piweV =Learn.learnOptions.getWeFeaturesMapforV().get(Step.pi);
		if (piweV ==null && Parse.parseOptions.getWeFeaturesMapforV().get(Step.pi)!=null)
			piweV = Parse.parseOptions.getWeFeaturesMapforV().get(Step.pi);

		WEFeature piweN =Learn.learnOptions.getWeFeaturesMapforN().get(Step.pi);
		if (piweN ==null && Parse.parseOptions.getWeFeaturesMapforN().get(Step.pi)!=null)
			piweN = Parse.parseOptions.getWeFeaturesMapforN().get(Step.pi);
		...
		WEFeature pdweV =Learn.learnOptions.getWeFeaturesMapforV().get(Step.pd);
		if (pdweV ==null && Parse.parseOptions.getWeFeaturesMapforV().get(Step.pd)!=null)
			pdweV = Parse.parseOptions.getWeFeaturesMapforV().get(Step.pd);

		WEFeature pdweN =Learn.learnOptions.getWeFeaturesMapforV().get(Step.pd);
		if (pdweN ==null && Parse.parseOptions.getWeFeaturesMapforV().get(Step.pd)!=null)
			pdweN = Parse.parseOptions.getWeFeaturesMapforV().get(Step.pd);

		pdwes.put("V", pdweV);
		pdwes.put("N",pdweN);

		WEFeature aiweV =Learn.learnOptions.getWeFeaturesMapforV().get(Step.ai);
		if (aiweV ==null && Parse.parseOptions.getWeFeaturesMapforV().get(Step.ai)!=null)
			aiweV = Parse.parseOptions.getWeFeaturesMapforV().get(Step.ai);

		WEFeature aiweN =Learn.learnOptions.getWeFeaturesMapforN().get(Step.ai);
		if (aiweN ==null && Parse.parseOptions.getWeFeaturesMapforN().get(Step.ai)!=null)
			aiweN = Parse.parseOptions.getWeFeaturesMapforN().get(Step.ai);
		...
		WEFeature acweV =Learn.learnOptions.getWeFeaturesMapforV().get(Step.ac);
		if (acweV ==null && Parse.parseOptions.getWeFeaturesMapforV().get(Step.ac)!=null)
			acweV = Parse.parseOptions.getWeFeaturesMapforV().get(Step.ac);

		WEFeature acweN =Learn.learnOptions.getWeFeaturesMapforN().get(Step.ac);
		if (acweN ==null && Parse.parseOptions.getWeFeaturesMapforN().get(Step.ac)!=null)
			acweN = Parse.parseOptions.getWeFeaturesMapforN().get(Step.ac);
		...
		for(Step step:Step.values()){
			if(featureSets.containsKey(step)){
				switch(step){
				case pi: steps.add(new PredicateIdentifier(featureSets.get(Step.pi), piwes )); break;
				case pd: steps.add(new PredicateDisambiguator(featureSets.get(Step.pd),predicateReference, pdwes)); break;
				case ai: steps.add(new ArgumentIdentifier(featureSets.get(Step.ai), aiwes)); break;
				case ac: steps.add(new ArgumentClassifier(featureSets.get(Step.ac),argLabels, acwes)); break;
				}
			}
		}
	}
